...
def Wavefunction(obj, **kwargs):
    ...
    if(type(obj) == QuantumCircuit ):
        # Old: statevec = execute( obj, S_simulator, shots=1 ).result().get_statevector()
...
def Measurement(quantumcircuit, **kwargs):
    ...
    if 'column' in kwargs:
        NL = kwargs['column']
    # Old: M1 = execute(quantumcircuit, M_simulator, shots=S).result().get_counts(quantumcircuit)
...
def E_Expectation_Value( qc, Energies ):
    ...
    '''
    # Old: SV = execute( qc, S_simulator, shots=1 ).result().get_statevector()
...
def VQE_EV(params, Ansatz, H, EV_type, **kwargs):
    ...
    for k in np.arange( len(Hk) ):
        ...
        qc= QuantumCircuit(q,c)
        Ansatz( qc, q, params )
        # Old: sv0 = execute( qc, S_simulator, shots=1 ).result().get_statevector()
